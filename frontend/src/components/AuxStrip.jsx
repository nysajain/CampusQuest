    ...
    <div className="border-t border-white/8 bg-white/3">
      <div className="px-4 py-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] font-body font-semibold tracking-widest text-white/40 uppercase">
            Zone Aux
          </span>
          <span className="text-white/20 text-[10px]">·</span>
          <span className="text-[10px] text-white/40">{zone.listeners} listening</span>
        </div>

        <div className="flex items-center gap-2 mb-1">
          <Music2 size={13} className="text-emerald-400 flex-shrink-0" />
          <p className="text-sm font-body font-medium text-white truncate flex-1">
            {zone.now_playing}
          </p>
        </div>

        {zone.queue?.length > 0 && (
          <p className="text-[11px] text-white/35 mb-3 pl-[21px] truncate">
        ...
        <button
          onClick={() => setOpen(o => !o)}
          className="flex items-center gap-1.5 text-xs font-body font-medium text-asu-gold/80
        ...
        {open && (
          ...
          >
            <div className="px-4 py-3 space-y-2">
              <p className="text-[11px] text-white/40 font-body">quick picks</p>
              <div className="grid grid-cols-2 gap-1.5">
                {(QUICK_PICKS[zone.id] || []).map(s => (
                  <button
                    key={s}
                    onClick={() => handleAdd(s)}
                    className="text-left text-[11px] font-body text-white/70 bg-white/5
                               hover:bg-asu-gold/10 hover:text-asu-gold border border-white/10
                               hover:border-asu-gold/30 rounded-lg px-2.5 py-2 transition-all
              ...
              <p className="text-[11px] text-white/40 font-body pt-1">or type your own</p>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white/5 border border-white/15 rounded-xl px-3 py-2
                             text-sm text-white placeholder-white/25 font-body
